...
class Human(data.Dataset):
    def __init__(self, exp_args):
        ...
        if "supervisely" in self.datasetlist:
            ...
            self.datasets["supervisely"] = PortraitSeg(
                ImageRoot, AnnoRoot, ImgIds_Train, ImgIds_Test, self.exp_args
            )
        ...
        if "EG1800" in self.datasetlist:
            ...
            self.datasets["eg1800"] = PortraitSeg(
                ImageRoot, AnnoRoot, ImgIds_Train, ImgIds_Test, self.exp_args
            )
        ...
        if "supervisely_face_easy" in self.datasetlist:
            ...
            self.datasets["supervisely_face_easy"] = PortraitSeg(
                ImageRoot, AnnoRoot, ImgIds_Train, ImgIds_Test, self.exp_args
            )
        ...
        if "supervisely_face_difficult" in self.datasetlist:
            ...
            self.datasets["supervisely_face_difficult"] = PortraitSeg(
                ImageRoot, AnnoRoot, ImgIds_Train, ImgIds_Test, self.exp_args
            )
        ...
        if "MscocoBackground" in self.datasetlist:
            ...
            AnnoRoot = (
                self.data_root
                + "mscoco2017/annotations/person_keypoints_{}.json".format(dataType)
            )
            ...
            self.datasets["background"] = PortraitSeg(
                ImageRoot, AnnoRoot, ImgIds_Train, ImgIds_Test, self.exp_args
            )
    ...
    def __getitem__(self, index):
        ...
        if self.task == "seg":
            input_ori, input, output_edge, output_mask = self.datasets[subset][
                subsetidx
            ]
            return (
                input_ori.astype(np.float32),
                input.astype(np.float32),
                output_edge.astype(np.int64),
